...
export const queries = {
  ...
  getWardenGatepassDetails:"SELECT UM.name,(SELECT COUNT(*)FROM  gps_db.gps_db.gps_gatepassmaster as GM WHERE GM.send_approval_to = UM.user_id and GM.status in ('Pending') and GM.applied_date=FORMAT(GetDate(), 'yyyy-MM-dd')) as pending_requests,(SELECT COUNT(*)FROM  gps_db.gps_db.gps_gatepassmaster as GM WHERE GM.send_approval_to = UM.user_id and GM.approved_or_rejected_by!='' and GM.approved_or_rejected_date=FORMAT(GetDate(), 'yyyy-MM-dd'))as serviced_requests FROM gps_db.gps_db.gps_usersmaster as UM where UM.role_id=2;",
